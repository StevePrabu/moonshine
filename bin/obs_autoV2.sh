...
## run template script
script="${MYBASE}/queue/autoV2_${obsnum}.sh"
cat ${base}bin/autoV2.sh | sed -e "s:OBSNUM:${obsnum}:g" \
                                -e "s:SCRATCHID:${scratchID}:g" \
                                -e "s:CAL:${cal}:g" \
                                -e "s:BASE:${MYBASE}:g"> ${script}

output="${base}queue/logs/autoV2_${obsnum}.o%A"
error="${base}queue/logs/autoV2_${obsnum}.e%A"
